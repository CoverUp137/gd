...
install_depend(){
    ...
    # 模块依赖
    pip3 install qrcode Telethon requests Pillow python-socks async_timeout prettytable
    ...
    if [ -f ${QLMainPath}/config/extra.sh ];then
        if [ `grep "#机器人所需依赖" ${QLMainPath}/config/extra.sh` ];then
            ...
            echo "pip3 install qrcode Telethon requests Pillow python-socks async_timeout prettytable" >>${QLMainPath}/config/extra.sh
